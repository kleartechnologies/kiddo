 ...
 *              arrives at the home's edge, the home does not move, a wrong
 *              join moves nothing, and nothing moves once it has settled.
 *
...
const ONLY = ARGS.find((arg) => arg.startsWith("--only="))?.slice(7).split(",") ?? null;
...
async function counting(cdp, sessionId) {
  ...
  for (const viewport of screens) {
    ...
    for (const index of BOARDS.counting) {
      ...
      if (!seen) {
        report(`${viewport.name.padEnd(27)} board ${index}`, ["no prompt line"]);
        continue;
      }
      /* Every part pops, or none does (a block of pips is not a group of
         things). Never a half-animated row. */
      if (seen.subjects !== 0 && seen.subjects !== seen.parts) {
        problems.push(`${seen.subjects} of ${seen.parts} parts pop`);
      }
      if (seen.subjects > 0) {
        ...
        if (order.some((t) => t === null)) problems.push("a thing never arrived");
      ...
      if (seen.maxHeightDrift > 0.5) problems.push(`row height moved ${seen.maxHeightDrift.toFixed(1)}px`);
      if (seen.grewBy > 0) problems.push(`grew the page ${seen.grewBy}px`);
      if (seen.said !== seen.saidAfter || !seen.said.trim()) problems.push("transcription changed");
      if ((await overflow(cdp, sessionId)) > 0) problems.push("horizontal scroll");
      ...
      report(
        `${viewport.name.padEnd(27)} board ${index}  ${String(seen.parts).padStart(2)} parts, ` +
          `${String(seen.subjects).padStart(2)} pop, row ±${seen.maxHeightDrift.toFixed(1)}px`,
        problems,
      );
    }
  }
  ...
  for (const index of BOARDS.counting) {
    ...
    if (!seen) { problems.push(`board ${index} missing`); continue; }
    if (seen.minOpacity < 0.99) problems.push(`board ${index} faded in`);
    if (seen.subjects !== 0 && seen.subjects !== seen.parts) problems.push(`board ${index} half`);
  }
...
async function celebrate(cdp, sessionId) {
  ...
  for (const viewport of screens) {
    ...
    const problems = [];
    if (!seen) { report(viewport.name, ["no celebration on the page"]); continue; }
    ...
    if (seen.maxScale > 1.08) problems.push(`scaled to ${seen.maxScale.toFixed(2)}`);
    if (Math.abs(seen.final.y) > 0.5 || Math.abs(seen.final.s - 1) > 0.01) problems.push("did not settle");
    if (seen.sparksSeen === 0 || seen.sparksSeen > 3) problems.push(`${seen.sparksSeen} marks`);
    if (seen.sparksVisible > 0) problems.push(`${seen.sparksVisible} marks left visible`);
    if (seen.title !== seen.titleAfter || !seen.title) problems.push("title changed");
    if (JSON.stringify(seen.buttons) !== JSON.stringify(seen.buttonsAfter)) problems.push("controls changed");
  ...
  if (!seen) problems.push("missing");
    ...
    if (seen.sparksSeen > 0) problems.push(`${seen.sparksSeen} marks under reduce`);
...
const WATCH_WALK = (id, homeId, ms) => `new Promise((done) => {
  ...
  let maxY = 0, minX = 0, escaped = 0, homeMoved = 0, grewBy = 0, backwards = 0, boardShift = 0;
  ...
  const frameFn = () => {
    ...
    const overlaps = (a, b) => a && b && a.left < b.right && a.right > b.left && a.top < b.bottom && a.bottom > b.top;
      ...
      maxY, minX, escaped, homeMoved, grewBy, backwards, boardShift, hiddenFrames,
...
async function walkOne(cdp, sessionId) {
  ...
  const left = page.nodes.find((n) => n.side === "left" && n.state !== "matched");
  const rights = page.nodes.filter((n) => n.side === "right" && n.state !== "matched");
  ...
  for (const right of rights) {
    ...
    if (now?.state === "matched") {
      const watched = await evaluate(cdp, sessionId, WATCH_WALK(left.id, right.id, 2100));
      ...
      return { left, right, before, watched, later, wrongMoves, nodes: page.nodes, frame: page.frame };
    }
...
function judgeWalk(seen, problems) {
  ...
  if (!watched) { problems.push("no walker on the matched animal"); return; }
  if (seen.wrongMoves.some((d) => d > 0.5)) problems.push("a wrong join moved the animal");
  if (watched.final.x <= 1) problems.push("never walked");
  if (watched.minX < -0.5 || watched.backwards > 0) problems.push("walked backwards");
  if (watched.maxY > 3) problems.push(`bobbed ${watched.maxY.toFixed(1)}px`);
  ...
  if (Math.abs(gap) > 1.5) problems.push(`stopped ${gap.toFixed(1)}px from its place in the home`);
  ...
  if (watched.homeArtLeft !== null && watched.finalBox.right > watched.homeArtLeft - 2) {
  ...
  if (watched.hiddenFrames > 0) problems.push(`painted over for ${watched.hiddenFrames} frames`);
  if (watched.homeMoved > 0.5) problems.push(`home moved ${watched.homeMoved.toFixed(1)}px`);
  if (watched.escaped > 0) problems.push(`left the board on ${watched.escaped} frames`);
  if (watched.grewBy > 0) problems.push(`grew the page ${watched.grewBy}px`);
  if (!later || Math.abs(later.x - watched.final.x) > 0.1 || Math.abs(later.y) > 0.1) {
    problems.push("moved after settling");
  }
  if (!/joined to/.test(watched.label ?? "")) problems.push("animal's name lost its state");
  if (!/joined to/.test(watched.homeLabel ?? "")) problems.push("home's name lost its state");
  if (before && Math.abs(before.x) > 0.5) problems.push("animal started displaced");
}
...
async function walk(cdp, sessionId) {
  ...
  for (const viewport of screens) {
    ...
    for (const index of BOARDS.walk) {
      ...
      if (page.activity && !page.activity.startsWith("general-knowledge.home-partners")) {
      ...
      if (page.nodes.some((n) => n.side === "right" && n.walker)) problems.push("a home can walk");
      if (page.nodes.length && Math.min(...page.nodes.map((n) => n.min)) < MIN_TOUCH) {
        problems.push(`node only ${Math.round(Math.min(...page.nodes.map((n) => n.min)))}px`);
      }
      ...
      const travelled = seen.watched ? `${seen.watched.final.x.toFixed(0)}px` : "—";
      const shifted =
  ...
  for (const right of rights) {
    ...
    if (now?.state === "matched") {
      ...
      const home = page.nodes.find((n) => n.side === "right" && n.id === right.id);
  ...
  else if (Math.abs(placed) > 1.5) problems.push(`animal ${placed.toFixed(1)}px from its place in the home`);
  await reduce(cdp, sessionId, false);
...
async function finishQuest(cdp, sessionId) {
  ...
  for (let taps = 0; taps < 80; taps++) {
    if (await evaluate(cdp, sessionId, `Boolean(document.querySelector("[data-magic='celebrate']"))`)) {
      return true;
    }
    const tiles = await evaluate(cdp, sessionId, TILES);
    /* A tried tile says so in its name; that is the same question, not a new one. */
    const now = tiles.map((t) => t.label.replace(/, already tried$/, "")).join("|");
    if (now !== names) { names = now; tried = 0; }
    const next = tiles[tried];
    if (!next) { await settle(cdp, sessionId, 600); continue; }
    tried += 1;
  ...
  return Boolean(await evaluate(cdp, sessionId, `document.querySelector("[data-magic='celebrate']")`));
}
...
async function round(cdp, sessionId) {
  ...
  for (const reducedMotion of [false, true]) {
    ...
    if (!reached) {
      report(reducedMotion ? "reduced motion: math quest" : "math quest", ["never reached the end"]);
      continue;
    }
    /* The lift waits for the card to fade in, so the watch is long enough
       to see it happen and then see it stop. */
    const seen = await evaluate(cdp, sessionId, WATCH_CELEBRATE(reducedMotion ? 500 : 2400));
    if (!seen) { report("math quest", ["no celebration figure"]); continue; }
    if (reducedMotion) {
      if (seen.moved || seen.maxScale > 1.001) problems.push("moved under reduced motion");
      if (seen.sparksSeen > 0) problems.push(`${seen.sparksSeen} marks under reduced motion`);
    } else {
      if (!seen.moved) problems.push("never lifted");
      if (seen.maxY > 10) problems.push(`lifted ${seen.maxY.toFixed(1)}px`);
      if (seen.maxScale > 1.08) problems.push(`scaled to ${seen.maxScale.toFixed(2)}`);
    ...
    if (!seen.buttons.some((b) => /play again/i.test(b))) problems.push("no play again");
    if (JSON.stringify(seen.buttons) !== JSON.stringify(seen.buttonsAfter)) problems.push("controls changed");
    if (seen.exposed > 0) problems.push(`${seen.exposed} drawings exposed`);
...
try {
  ...
  report(`console: ${trouble.length} problems`, trouble.length ? [trouble[0]] : []);
  console.log("");
} catch (error) {
  console.error(error);
  failures += 1;
