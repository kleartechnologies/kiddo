 ...
 * screen time need not be wasted time (the hero, with the price in the
 * open), here is the thing itself before another word is spent (half a
 * minute of real gameplay), here is your house on a Tuesday night (the four
 * photographs), the problem was never the screen (the turning point, feed
 * against KIDDO side by side), this is what we made instead (KIDDO and its
 * three worlds, and the rest of the shelf beside them), this is what an hour
 * of it looks like (four steps and the child's real doors), this is what it
 * gives you (the parents' half), here are other parents saying it in their
 * own words, and then — only then — what it costs and the way in.
 *
 * The gameplay comes second because a cold visitor believes footage before
 * copy, and the pain points still come before the rest of the product: a
 * parent who has not yet agreed there is a problem has no reason to read a
 * feature list, and a page that opens on features is a page they close.
 *
...
export default function LandingPage() {
  return (
    <Screen width="wide" detail="quiet">
      ...
      <LandingFooter />
      {/* On a phone, the way in follows the reader; see `StickyCta`. */}
      <StickyCta />
    </Screen>
  );
}
