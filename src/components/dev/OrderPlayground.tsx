...
import { OrderStage } from "@/components/games/engines/OrderStage";
 ...
 *
 * The `order` engine, on its own, with the reference activity in it. It is
 * here for the same reason `ConnectPlayground` is: so the engine can be
 * played, measured and reviewed before a Quest exists to hold it.
 *
 * It is deliberately not a game. It is not in `data/games.ts`, it has no route
 * under `/play`, and the home screen has never heard of it. When Order content
 * arrives in a real pack, that Quest will render this same `OrderStage` with
 * this same hook, and this page will still be here to test the engine against.
 *
...
/** 3, 4 and 5 tiles — the smallest ordering there is, and the widest one. */
...
export function OrderPlayground() {
  ...
  const challenge = useMemo(() => {
    ...
    const [board] = drawChallenges(orderReferenceActivity, {
      level,
      count: 1,
  ...
  return (
    ...
    <div className="flex flex-col gap-4 py-4 sm:gap-6 sm:py-6 [@media(max-height:44rem)]:gap-2 [@media(max-height:44rem)]:py-2">
      ...
      {challenge && isKind(challenge, "order") ? (
        <OrderBoard key={`${challenge.id}#${deal}`} challenge={challenge} />
      ) : null}
    </div>
  );
}

/** Split out so the hook below always has an `order` challenge to run on. */
function OrderBoard({ challenge }: { challenge: ChallengeOf<"order"> }) {
  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-5">
      ...
      {/* The same thing in words, because the board's own state lives in
          labels nobody is focused on while a tile is being carried. */}
