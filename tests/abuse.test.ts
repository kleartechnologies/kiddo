...
test("N — security headers are configured, and the CSP names only what KIDDO uses", async () => {
  ...
  assert.deepEqual(frame.split(" ").sort(), [
    /* The hidden /__/auth/iframe the Google popup answers through, at both
       the addresses Firebase serves it on for this project: KIDDO's own
       origin, where `netlify.toml` proxies `/__/auth/*` and where
       `authDomain` points in production, and the project's own domain a
       build falls back to. Asserted again below. */
    "'self'",
  ...
  /* The fallback is not a loose string: it is spelled the way
     `firebaseConfig()` spells it when the environment does not override it,
     so moving Firebase projects fails here rather than silently blocking
     sign-in in a browser nobody is watching. */
  ...
  assert.ok(frame.includes("'self'"), "frame-src must allow the proxied /__/auth/iframe on KIDDO's own origin");
